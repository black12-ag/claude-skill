...
## For native Agent integration (Claude, Gemini, Codex)

Run once per project to make graphify always-on in Claude Code, Gemini CLI, and Codex CLI sessions:

```bash
cat << 'EOF' > .graphify_agent_rules.md
## graphify
This project uses graphify to maintain a persistent knowledge graph of the codebase architecture in `graphify-out/`.
This saves tokens and prevents hallucination by giving you a pre-computed map of all files.

**Mandatory Workflow:**
1. Before answering architecture questions or planning changes, READ `graphify-out/GRAPH_REPORT.md` (especially the "Community Hubs" and "God Nodes" sections).
2. If you need to trace a specific connection, use `graphify query "your question"` or `graphify path "A" "B"` using the bash tool.
3. NEVER guess how things are connected - check the graph first.
4. After you create, modify, or delete files, you MUST run `graphify --update` to re-extract relationships and keep the graph accurate for future turns.
EOF

cat .graphify_agent_rules.md >> CLAUDE.md
cat .graphify_agent_rules.md >> GEMINI.md
cat .graphify_agent_rules.md >> AGENTS.md
rm .graphify_agent_rules.md
```

This writes a `## graphify` section to `CLAUDE.md`, `GEMINI.md`, and `AGENTS.md` that instructs all AI agents to check the graph before answering codebase questions and rebuild it after code changes. No manual `/graphify` needed in future sessions.
